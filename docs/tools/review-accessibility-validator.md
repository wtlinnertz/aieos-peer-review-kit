# Review Lens: Accessibility — Validator

## Purpose

Evaluate the output of the accessibility review lens against the hard gates defined in `review-accessibility-spec.md`. Produce a PASS/FAIL judgment. Do not help, suggest improvements, or expand scope.

## Inputs

1. The accessibility lens output to evaluate
2. `docs/tools/review-accessibility-spec.md` — the authoritative rules
...
- No findings based on general assertions ("accessibility is important") or assumptions about unspecified behavior
- "No findings" is acceptable if justified (e.g., artifact has no user-facing components)
...
- Recommendations address the specific accessibility gap, not general best practices
- Recommendations do not expand scope beyond the artifact's defined boundaries
- "Consider accessibility" is not actionable — recommendations must specify what to do

## Additional Checks

- If the lens output claims "no findings," verify there is a justification (e.g., "artifact is backend-only with no user-facing components")
- No findings outside the accessibility domain (security findings, cost concerns, etc.)
...
- Do not evaluate the quality of the accessibility analysis itself — only whether the output meets structural and evidence requirements
